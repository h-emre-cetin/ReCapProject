...
namespace Business.Concrete
{
    public class CarImageManager : ICarImageService
    {
        ...
        {
            IResult result = BusinessRules.Run(CheckTheCarImageLimit(carImage.CarId));
            if (result!=null)
            {
                return result;
            }
        ...
        private IResult CheckTheCarImageLimit(int carId)
        {
            var result = _carImageDal.GetAll(c => c.CarId == carId).Count;
            if (result>5)
            {
                return new ErrorResult(Messages.ImageUploadFail);
            }
            return new SuccessResult();
        }
